...
static int init_gpio(const struct device *dev)
{
    struct my_uart_conf *conf = (struct my_uart_conf *)dev->config;
    int ret;
    ret = gpio_pin_configure_dt(&conf->gpio_spec, GPIO_INPUT | GPIO_INT_EDGE_RISING);
    if (ret)
        return ret;
    ret = gpio_pin_interrupt_configure_dt(&conf->gpio_spec, GPIO_INT_EDGE_TO_ACTIVE | GPIO_INT_EDGE_RISING);
    if (ret)
        return ret;
